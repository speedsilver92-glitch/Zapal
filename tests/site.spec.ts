...
test("floating contact button is hidden on desktop and shown on mobile", async ({ page }) => {
  // Desktop: hidden so it never overlaps footer / CTA content.
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto("/");
  await expect(page.locator(".floating-contact")).toBeHidden();
  // Mobile: shown, since the nav CTA lives behind the hamburger.
  await page.setViewportSize({ width: 390, height: 844 });
  await page.reload();
  await expect(page.locator(".floating-contact")).toBeVisible();
});

test("layout uses wide desktop space without overflow", async ({ page }) => {
  // Common desktop widths should use a healthy share of the viewport.
  for (const width of [1366, 1440, 1536, 1920]) {
    await page.setViewportSize({ width, height: 900 });
    await page.goto("/");
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
    const usage = await page.evaluate(() => document.querySelector(".container")!.getBoundingClientRect().width / window.innerWidth);
    expect(usage, `container usage at ${width}px`).toBeGreaterThan(0.78);
  }
  // Ultra-wide: content is intentionally capped, but must not overflow.
